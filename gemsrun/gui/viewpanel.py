...
class ViewPanel(QWidget):
    ...
    def dropEvent(self, ev: QDropEvent) -> None:
        ...
        self.drag_object_bitmap = None  # Clear the stored bitmap
        ev.accept()
    ...
    def create_nav_pics(self):
        ...
        for nav_type in ("NavLeft", "NavRight", "NavTop", "NavBottom"):
            actions = sorted(
                [action for action in self.View.Actions.values() if action.Trigger.startswith(nav_type)],
    ...
    def handle_object_drop(self, source_id: int, target_id: int, source_view_id: int = -1):
        ...
        valid_drop = any(
            action for action in target_actions if action.Enabled and action.Trigger.replace(" ", "") == trigger
        )
