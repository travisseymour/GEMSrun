...
class CrossPlatformAudioPlayer(QObject):
    ...
    def _detect_available_backends(self) -> list:
        ...
        try:
            ...
            if test_output is not None and test_player is not None and len(audio_devices) > 0:
                ...
                log.debug(
                    f"QMediaPlayer backend not available - output:{test_output is not None}, player:{test_player is not None}, devices:{len(audio_devices)}"
                )
        ...
        for cmd in commands:
            try:
                ...
                if cmd == "afplay":
                    ...
                    log.debug("Found afplay command on macOS")
                    return True
                elif cmd == "powershell":
                    # Check if powershell is available
                    result = subprocess.run([cmd, "-Command", "Get-Host"], capture_output=True, timeout=2)
                    if result.returncode == 0:
                        log.debug("Found powershell command on Windows")
                        return True
                else:
    ...
    def play(self) -> bool:
        ...
        try:
            ...
            if remaining := [b for b in preferred_order if b != backend]:
                log.info("Falling back to next available backend")
        ...
        try:
            ...
            try:
                ...
                if len(audio_devices) == 0:
                    ...
                    raise RuntimeError("No audio output devices available") from e
    ...
    def _play_with_qsoundeffect(self) -> bool:
        ...
        try:
            ...
            self.player.playingChanged.connect(
                lambda *args: self._on_playing_changed(args[0] if args else self.player.isPlaying())
            )
